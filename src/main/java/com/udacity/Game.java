...
public class Game {
     ...
     */
//    Edit the checkGameWinner method.
//    Hardcode in 2d array for checking diagonals and use check inside double for loop for rows and columns.
//    After all the checks, count the number of spaces not empty and if they equal size of the board then return as tie.
//    grid.length*grid.length to count all the spaces in the board.
        ...
        char [] symbols = {'x', 'o'}; 
	        ...
	        for (int i = 0; i< symbols.length; i++) {
	        	...
	        	for (int j = 0; j < grid.length; j++) {
	        		if ( grid[j][0] == symbols[i] && grid[j][1] == symbols[i] && grid[j][2] == symbols[i]) {    
	        			return message[i];    	         
	        		}
	        		if ( grid[0][j] == symbols[i] && grid[1][j] == symbols[i] && grid[2][j] == symbols[i]) {		
	        			return message[i];		
	        		}			
	        	}
	        }
 	
    		int counter = 0; // this variable is for counting not empty spaces
//    		char emptySpace = '_';
